...
class UDPService():
    ...
    def __init__(self,host='localhost',post=8080):#UDP服务构造函数，在中控初始化时调用，以便先设置好套接字并开始连接
        self.HOST=host
        self.PORT=port
        ...
        self.server.connect((HOST,PORT))#连接到指定客户端的IP地址和端口
    ...
    def sendFrame(self,frame):#发送帧函数，请在中控循环获取每一帧的摄像头数据并解析和显示画面的函数中循环调用，传入cap.read()函数获取的frame
        try:
            ...
            self.server.sendall(imgData)#发送全部二进制串
